// 购物车状态mergeCart
import { mergeLocalCart, getNewCartGoods, findCartList, insertCart, deleteCart, updateCart, checkAllCart } from '@/api/cart'
export default {
  namespaced: true,
  state: () => {
    return {
  ...
  getters: {
    ...
    selectedAmount (state, getters) {
      return (getters.selectedList.reduce((p, c) => p + (c.nowPrice * 100 * c.count), 0) / 100).toFixed(2)
    },
    // 是否全选
  ...
  actions: {
    ...
    async mergeLocalCart (ctx) {
      ...
      const cartList = ctx.getters.validList.map(({ skuId, selected, count }) => {
        console.log(skuId, selected, count)
