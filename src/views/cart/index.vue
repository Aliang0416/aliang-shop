              ...
              <td>
                <div class="goods">
                  ...
                  <div>
                    ...
                    <!-- 选择规格组件 -->
                    <cart-sku :attrsText="item.attrsText" :skuId="item.skuId" @change="$event=>updateCartSku(item.skuId,$event)" />
                  </div>
                </div>
              </td>
              <td class="tc">
            ...
            <tr v-for="i in $store.getters['cart/invalidList']" :key="i">
              ...
              <td>
                <div class="goods">
                  ...
                  <div>
                    <p class="name ellipsis">{{i.attrsText}}</p>
                    <p class="attr">颜色：粉色 尺寸：14cm 产地：中国</p>
                  </div>
                </div>
              </td>
              <td class="tc"><p>&yen;200</p></td>
              <td class="tc">1</td>
...
export default {
  ...
  setup () {
    ...
    const batchDeleteCart = (isClear) => {
      if (!isClear) {
        if (store.getters['cart/selectedTotal'] === 0) return
        Confirm({ text: '您确定从购物车删除选中的商品吗？' }).then(() => {
          store.dispatch('cart/batchDeleteCart', isClear)
        }).catch(e => {})
      } else {
        if (store.getters['cart/invalidList'].length === 0) return
        Confirm({ text: '您确定从购物车删除失效的商品吗？' }).then(() => {
          store.dispatch('cart/batchDeleteCart', isClear)
        }).catch(e => {})
      }
    }
