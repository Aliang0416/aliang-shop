...
const getCommentInfo = (props) => {
  ...
  findCommentInfoByGoods(props.goodsId).then(data => {
...
export default {
  ...
  setup (props) {
    ...
    watch(reqParams, async () => {
      const data = await findCommentListByGoods(props.goodsId, reqParams)
    ...
    return { commentInfo, changeTag, currTagIndex, changeSort, reqParams, commentList, formatSpecs, formatNickname, total, changePager }
  },
